function [a_pos,eu_1,eu_1_alpha,eu_2,eu_2_alpha,x_pos_seat,z_pos_seat,x_pos_br,z_pos_br,Lz_br] = PlotInterior_vav(vehicle,int_boundary_f_new,int_boundary_r_new,a_1,a_2)
%% Description:
...
% Version: Matlab2020b
%-------------
% Description: %This function calculates the positions of the seats for vis-a-vis
% ------------
...
eu_1 = [0; pi+2*pi/360*vehicle.Input.angle_br_max(1) ;0];
eu_1_alpha = [0; pi+2*pi/360*vehicle.Input.angle_br_min(1) ;0];
eu_2 = [0; -2*pi/360*vehicle.Input.angle_br_max(2);0];
eu_2_alpha = [0; -2*pi/360*vehicle.Input.angle_br_min(2);0];

x1_seat = int_boundary_f_new(vehicle.Input.int_height + 1)-vehicle.Input.seat_depth(1);
x2_seat = int_boundary_r_new(vehicle.Input.int_height + 1);
x1_br   = x1_seat;
x2_br   = x2_seat+vehicle.Input.seat_depth(2);
...
x_pos_seat = [x1_seat  x2_seat-vehicle.Input.seat_adjustment_x(2)];
