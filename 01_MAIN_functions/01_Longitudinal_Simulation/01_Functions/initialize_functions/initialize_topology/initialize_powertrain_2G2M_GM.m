function [v] = initialize_powertrain_2G2M_GM(v, Par)
...
% Version: Matlab2020b
%-------------
% Description: This functions initializes a powertrain configuration for 2 machines and 2 gearboxes on front axle and 1 machine and 1 gearbox on rear axle.
...
if ~isempty(v.Input.eta_gearbox_r) && all(~isnan(v.Input.eta_gearbox_r))
    v.LDS.GEARBOX{2}.eta                = v.Input.eta_gearbox_r*Par.LDS.eta_differential;                % efficiency of gearbox
else
    v.LDS.GEARBOX{2}.eta                = Par.LDS.eta_gearbox*Par.LDS.eta_differential;                  % efficiency of gearbox
end
