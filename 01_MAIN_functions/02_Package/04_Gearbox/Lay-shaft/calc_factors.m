function gearbox = calc_factors(shaft, gearbox)
%% 1) Description:
...
if (shaft==1)
    ...
    l = gearbox.bearings_1.l_1;                     % Distance between bearings on shaft 1 [mm]
...
elseif (shaft==2)
    ...
    l = gearbox.bearings_2.l_2;                     % Distance between bearings on shaft 2 [mm]
...
f_sh = (F_m/b)*0.023*(abs(0.7+(K_prime*l*s/(d_1^2))*((d_1/d_sh_r)^4))+0.3)*((b/d_1)^2);
...
%% 4) Output assignment:
if (shaft==1)
    gearbox.gears_12.beta_b_1 = beta_b;                 % Base helix angle of first stage in rad
...
elseif (shaft==2) 
    gearbox.gears_34.beta_b_2 = beta_b;                 % Base helix angle of second stage in rad
