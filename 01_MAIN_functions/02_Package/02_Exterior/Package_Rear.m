function [vehicle] = Package_Rear(vehicle,drivetrain_type,Parameters)
%% Description:
...
% Version: Matlab2020b
%-------------
% Description:  This function calculates the package of the rear area of the vehicle 
...
wagontype   =   2;    %1=Front, 2=Rear (To use same functions for front and rear wagon)
psi         =   vehicle.Input.psi_motor(2); %Read Angle of Motor to Gear in Degree

%% 0) Measurements:
%Minimum distance between plate and cross member along the X direction (in mm)
CX_plate_cross_member_rear=Parameters.bumper.dimensions.CX.plate_cross_member_rear;
...
%% 4) Calculate bumper x-pos.
p_bumper_temp_r                     =   -(vehicle.Input.dist_wheelhouse2bumper_r+vehicle.dimensions.wheelhouse_width(wagontype)/2); % temporary bumper pos. = wheelhouse pos. + distance wheelhouse-bumper
vehicle.dimensions.p_bumper{2}(1,1) =   p_bumper_temp_r-vehicle.dimensions.p_bumper{wagontype}(2,1)*0.5; % bumper x-pos = temporary pos. - half length bumper
...
elseif drivetrain_type(wagontype)==1 %motor with parallel gear
    vehicle.dimensions.p_powertrain{2}=cat(1,vehicle.dimensions.p_motor_paral{2},vehicle.dimensions.p_gear_paral{2});
    p_bodies=["vehicle.dimensions.p_powertrain{2}","vehicle.dimensions.p_powerel{2}"];
    
elseif  drivetrain_type(wagontype)==2 %motor with coaxial gear (planetary)
    vehicle.dimensions.p_powertrain{2}=cat(1,vehicle.dimensions.p_motor_coax{2},vehicle.dimensions.p_gear_coax{2});
    p_bodies=["vehicle.dimensions.p_powertrain{2}","vehicle.dimensions.p_powerel{2}"];
    
elseif drivetrain_type(wagontype)==3 %two motors in coaxial position
    vehicle.dimensions.p_powertrain_1{2}=cat(1,vehicle.dimensions.p_motor_nax{2}(1,:),vehicle.dimensions.p_motor_nax{2}(2,:),vehicle.dimensions.p_gear_nax{2}(1,:),vehicle.dimensions.p_gear_nax{2}(2,:));
    vehicle.dimensions.p_powertrain_2{2}=cat(1,vehicle.dimensions.p_motor_nax{2}(3,:),vehicle.dimensions.p_motor_nax{2}(2,:),vehicle.dimensions.p_gear_nax{2}(3,:),vehicle.dimensions.p_gear_nax{2}(2,:));
    p_bodies=["vehicle.dimensions.p_powertrain_1{2}","vehicle.dimensions.p_powertrain_2{2}","vehicle.dimensions.p_powerel{2}"];   %every body included into the rear wagon, which is part of border to interior and block at crash

elseif drivetrain_type(wagontype)==4 %motor with coaxial gear (layshaft)
    vehicle.dimensions.p_powertrain{2}=cat(1,vehicle.dimensions.p_motor_coax{2},vehicle.dimensions.p_gear_coax_ls{2});
    p_bodies=["vehicle.dimensions.p_powertrain{2}","vehicle.dimensions.p_powerel{2}"];
end
...
for i =1:length(Layer_ypos)
    if strcmp(Layer_names{1,i},'-')
        Boundary_Rear_max=zeros(length(Layer_boundary{1,1}),1);
    else
        [vehicle,Boundary_Rear_max] = Package_Layer(vehicle,Layer_names{1,i},psi,wagontype,1);
    end
    
    % Adding bumper distance to front axle in x-direction from Package_Layer_Calculate function
    Boundary_Rear_max=Boundary_Rear_max+(vehicle.dimensions.p_bumper{wagontype}(1,1)+0.5*vehicle.dimensions.p_bumper{wagontype}(2,1));
    Layer_boundary{1,i}=Boundary_Rear_max;
end

%% b) Select bodies that do not need block-building and determine boundary
p_bodies2=["vehicle.dimensions.p_wheelhouse_left{2}","vehicle.dimensions.p_wheelhouse_right{2}"];
...
for i =1:length(Layer_ypos2)
    if strcmp(Layer_names2{1,i},'-')
        Boundary_Rear_max2=zeros(length(Layer_boundary2{1,1}),1);
    else
        [vehicle,Boundary_Rear_max2] = Package_Layer(vehicle,Layer_names2{1,i},psi,wagontype,0);
    end
    
    % Adding bumper distance to front axle in x-direction from Package_Layer_Calculate function
    Boundary_Rear_max2  =   Boundary_Rear_max2+(vehicle.dimensions.p_bumper{wagontype}(1,1)+0.5*vehicle.dimensions.p_bumper{wagontype}(2,1));
    Layer_boundary2{1,i}=   Boundary_Rear_max2;
end

%% c) Merge boundaries and select maximum
...
for i=1:length(Layer_ypos3)-1
    ...
    if isempty(idx1) || length(idx1)==length(Layer_ypos)
        if ismember(idx2(end),idx2_help)
    ...
    if strcmp(Layer_names3{1,i},'-')
        Layer_boundary3{1,i}(Layer_boundary3{1,i}==0)=Layer_boundary2{1}(end);
    end
end
...
vehicle.dimensions.p_boundaries_r   =   p_boundaries;
...
vehicle.dimensions.p_boundaries_r_pow=  p_boundaries_pow;

%% 6) Update bumper position and overhang
% update bumper pos if temp. pos. changed
vehicle.dimensions.p_bumper{2}(1,1)     =   min([vehicle.dimensions.p_bumper{2}(1,1)  p_boundaries{3}(end)-0.5*vehicle.dimensions.p_bumper{2}(2,1)]);

% overhang = bumper pos + half bumper length
vehicle.dimensions.GX.vehicle_overhang_r=   -(vehicle.dimensions.p_bumper{wagontype}(1,1)-0.5*vehicle.dimensions.p_bumper{wagontype}(2,1))+CX_plate_cross_member_rear;
...
Boundary_Rear_Surf=cell(3,length(p_bodies)*2);
...
%Write all bodies and their edge y-positions in Boundary_Rear_Surf
for i=1:length(p_bodies)
    Boundary_Rear_Surf{1,i*2-1}=p_bodies(i);
    Boundary_Rear_Surf{1,i*2}=p_bodies(i);
    
    if drivetrain_type(wagontype)==1 && strcmp(p_bodies(i),'vehicle.dimensions.p_powertrain{2}')
        % Parallel powertrain
        ...
        Boundary_Rear_Surf{2,i*2-1}=eval(position_body_first_obj)-0.5*eval(width_body_first_obj);
        Boundary_Rear_Surf{2,i*2}=eval(position_body_last_obj)+0.5*eval(width_body_last_obj);
        
    elseif (drivetrain_type(wagontype)==2 || drivetrain_type(wagontype)==3) && (strcmp(p_bodies(i),'vehicle.dimensions.p_powertrain{2}') || strcmp(p_bodies(i),'vehicle.dimensions.p_powertrain_2{2}'))
    ...
    elseif strcmp(p_bodies(i),'vehicle.dimensions.p_powertrain_1{2}')
    ...
    elseif drivetrain_type(wagontype)==4 && strcmp(p_bodies(i),'vehicle.dimensions.p_powertrain{2}')
        ...
        Boundary_Rear_Surf{2,i*2-1}=eval(position_body)-0.5*eval(width_body);
        Boundary_Rear_Surf{2,i*2}=eval(position_body)+0.5*eval(width_body);
    end
end

%All positions of Layers
Layer_ypos=zeros(1,length(p_bodies)*2); %Preallocate y_pos string
Layer_ypos(1,:)=[Boundary_Rear_Surf{2,1:length(p_bodies)*2}]; %Use Values of bodies
...
for i=1:length(Layer_ypos)
    ...
    for j=1:length(p_bodies)
        ...
        if Boundary_Rear_Surf{2,j*2-1}<=Layer_ypos(i) && Boundary_Rear_Surf{2,j*2}>Layer_ypos(i) %Compare if body is at this y-position
            if strcmp(names,'-')
                names=Boundary_Rear_Surf{1,j*2-1}; %for the first time writing a name in the string
            else
                names=[names,Boundary_Rear_Surf{1,j*2-1}]; %for every further name
            end
        end
