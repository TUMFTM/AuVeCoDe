...
for p = 1 : opt.popsize
    for i = 1:opt.numVar
        ...
        variable_matrix(p,i)=pop(p).var(i);
    end
    for i = 1:opt.numObj
        fprintf(fid, '%g\t', pop(p).obj(i) );
        objective_matrix(p,i)=pop(p).obj(i);
    end
    for i = 1:opt.numCons
...
[filepath,name,ext] = fileparts(which('Run_AuVeCoDe')); % Main Folder
path_folder=fullfile(filepath,'06_Results'); % Result folder
path_var= fullfile(path_folder,'variable_matrix');
path_obj= fullfile(path_folder,'objective_matrix');

recycle on
save(path_var,'variable_matrix')
save(path_obj,'objective_matrix')
