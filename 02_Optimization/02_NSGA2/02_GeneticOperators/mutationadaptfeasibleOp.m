function pop = mutationadaptfeasibleOp(opt, pop, state)
...
for ind = 1:length(pop)
    for k = 1:100
        mutant = fun( pop(ind), opt, state, fraction, mutationopt);
        % Make sure mutant is feasible w.r.t. linear constraints else do not accept
        if all(opt.A * mutant.var' < opt.b')
            child = mutant;
            break;
        else
            child = pop(ind);
    ...
    %% Berücksichtigung von IntCon
    for v = 1:nVar
        if( opt.vartype(v) == 2)           
            if rand>=0.5
                child.var(v) = floor(child.var(v));
            else
                child.var(v) = ceil(child.var(v));
            end
        end
    end
